...
Molecule::Molecule() {
	...
	ptr=NULL;
#endif
	time=Run::time.current;// particle time 
	dt=Run::time.step;// particle timestep
	target=NULL;
	energy=0.0;
}
...
void Molecule::Temperature(REAL temperature //!< temperature in K
	...
	REAL // Add energy per internal degree of freedom:
		...
		dof=DOF(cp),//=2*cv/R=degrees of freedom
		dofi=DOFI(dof); // internal degrees of freedom 
	InternalEnergy(0.5*energy*dofi);
}
...
void Molecule::Move() {
	REAL
		mass=species[type].Mass();
	if (mass<SMALL) {
		cerr<<"MOLECULAR MASS "<<mass<<" FOR MOLECULE TYPE "<<type<<" OUT OF "<<Species::nspecies<<" IS TOO SMALL\n";return;
	}
	for (int i=0; i<DIM; i++) {
//+		REAL
