...
class Domain {
	...
	REAL bounds[2][DIM]; //xmin[DIM],xmax[DIM];//!< domain bounds
	...
	inline void setMinBound(int i, REAL x){ bounds[0][i]=x;};
	inline void setMaxBound(int i, REAL x){ bounds[1][i]=x;};
	inline REAL minBound(int i){ return bounds[0][i];};
	inline REAL maxBound(int i){ return bounds[1][i];};
...
//-	void load(char *filename, List<Molecule> *molecules, REAL xmin[], REAL xmax[]);
    ...
    */
	 ...
	 * Also estimates time to the next collision.
	 */
